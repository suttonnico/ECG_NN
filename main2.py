...
def PCA(lats, cantDim):
    ...
    # plt.figure()
    # plt.plot(np.matrix.transpose(lats)[4011])
    # plt.plot(np.matrix.transpose(latidosRec)[4011])
    # plt.title('Latido original y reconstruido')
    # plt.show()
    # sum1= 0
    # for i in range(0,len(lats)):
    #     sum1 += (np.var(np.matrix.transpose(lats)[i]))
    # sum2 = 0
    # for i in range(0,len(lats)):
...
def load_data(N, V, S, J):
    ...
    tam = 50  # Tamano del latido que tomo a la izquierda y a la derecha de la anotacion
    ...
    plt.figure()
    plt.plot(latidosNorm[43655])
    plt.title('Latido nodal prematuro')
    plt.show()
...
[N, V, S, J, N_left, V_left, S_left, J_left, training_set_N, training_set_V, training_set_S, training_set_J, test_set_N,
 test_set_V, test_set_S, test_set_J] = load_data(120, 40, 40, 40)
...
backpropagation(
    ...
    print_rate=1000,
    input_layer_dropout=0,  # Dropout fraction of the input layer
    hidden_layer_dropout=0,  # Dropout fraction of in the hidden layer(s)
)
test_set = []
wrn = 0
...
settings = {"n_inputs": tam, "layers": [(36, sigmoid_function), (16, sigmoid_function), (1, sigmoid_function)]}
network = NeuralNet(settings)
training_set = []
for i in range(0, N):
    training_set = np.append(training_set, Instance(training_set_N[i], [1]))
for i in range(0, V):
    training_set = np.append(training_set, Instance(training_set_V[i], [0]))
for i in range(0, S):
    training_set = np.append(training_set, Instance(training_set_S[i], [0]))
for i in range(0, J):
    training_set = np.append(training_set, Instance(training_set_J[i], [0]))
print len(training_set)
test_set = []
for i in range(0, 100):
    test_set = np.append(test_set, Instance(test_set_N[i], [1]))
for i in range(0, 100):
    test_set = np.append(test_set, Instance(test_set_V[i], [0]))
for i in range(0, 100):
    test_set = np.append(test_set, Instance(test_set_S[i], [0]))
for i in range(0, 100):
    test_set = np.append(test_set, Instance(test_set_J[i], [0]))
